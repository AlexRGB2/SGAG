...
export const addPago = async (req, res) => {
    ...
    for (let i = 0; i < productos.length; i++) {
        const producto = await products.findOne({ nombre: productos[i].nombre });
        if (!producto) return res.json({ msg: "product not found" });
        if (producto.stock < productos[i].cantidad) return res.json({ msg: "stock not found" });
    }

    for (let i = 0; i < productos.length; i++) {
        const  data = await products.findOne({ nombre: productos[i].nombre }, {stock: true});
        await products.updateOne({nombre: productos[i].nombre},{ $set: { stock: (data.stock - productos[i].cantidad) } })
    }

    const nombres = productos.map(x => x.nombre);

    const pro = await products.find({ nombre: { $in: nombres } }, { nombre: true, precio: true, _id: false })
    const precios = {};
